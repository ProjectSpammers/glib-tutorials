...
void
send_notification(int argc, char *argv[]) {
    ...
    GVariant *params = g_variant_new("(susssasa{sv}i)", "Ankara", 0, "computer", "Notification from C",
                                     "Hello, this was sent via D-Bus.", NULL, NULL, 5000);

    gchar *str = NULL;
    g_variant_get(params, "(susssasa{sv}i)", &str, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    g_print("Getting a single s: %s\n\n", str);

    g_free(str);

    GVariant *results = g_dbus_connection_call_sync(connection, "org.freedesktop.Notifications",
