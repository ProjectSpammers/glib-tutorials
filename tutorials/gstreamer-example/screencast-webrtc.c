...
static void
send_sdp_to_peer(const gchar *type, const gchar *sdp_string) {
...
static void
on_ice_gathering_state_change(GstElement *webrtc, guint mlineindex, gchar *candidate, gpointer user_data) {
...
static void
start_stream(guint32 id, ScreencastWebRTCState *state) {
    ...
    char *pipeline_str = g_strdup_printf(
        ...
        "pulsesrc device=GStreamer_Yayin.monitor do-timestamp=true buffer-time=200000 ! "
        "audioconvert ! "
        "audioresample ! "
        "opusenc ! "
        "rtpopuspay pt=97 ! "
        "queue ! sendrecv. ",
        id,
        state->is_sound_excluded > 0 ? "GStreamer_Yayin.monitor"
    ...
    // --- STUN & TURN SUNUCULARINI EKLEME ---

    // 1. STUN Server Ekleme (Örnek)
    g_object_set(state->webrtcbin, "stun-server", "stun:stun.relay.metered.ca:80", NULL);
    // 2. TURN Server Ekleme (Örnek)
    g_signal_emit_by_name(state->webrtcbin, "add-turn-server", "turns://test:test@test.com:443?transport=tcp", NULL);
    g_signal_connect(state->webrtcbin, "on-negotiation-needed", G_CALLBACK(on_negotiation_needed), state);
    g_signal_connect(state->webrtcbin, "notify::ice-gathering-state", G_CALLBACK(on_ice_gathering_state_change), state);
...
static void
on_start_response(GDBusConnection *conn, const gchar *sender, const gchar *path, const gchar *iface,
...
static void
start_screencast(ScreencastWebRTCState *state) {
    ...
    GVariant *ret   = g_dbus_connection_call_sync(
        ...
        g_variant_new("(osa{sv})", state->session_path, "", &opts), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    ...
    g_dbus_connection_signal_subscribe(state->connection, PORTAL_BUS_NAME, REQUEST_INTERFACE, "Response", req_path,
                                       NULL, G_DBUS_SIGNAL_FLAGS_NONE, on_start_response, state, NULL);
    g_free(req_path);
    g_free(token);
}

static void
on_select_response(GDBusConnection *conn, const gchar *sender, const gchar *path, const gchar *iface,
                   const gchar *signal, GVariant *params, gpointer user_data) {
    ScreencastWebRTCState *state = user_data;
    guint32                response_code;
...
static void
select_sources(ScreencastWebRTCState *state) {
    ...
    GVariant *ret   = g_dbus_connection_call_sync(
        ...
        g_variant_new("(oa{sv})", state->session_path, &opts), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    ...
    g_dbus_connection_signal_subscribe(state->connection, PORTAL_BUS_NAME, REQUEST_INTERFACE, "Response", req_path,
                                       NULL, G_DBUS_SIGNAL_FLAGS_NONE, on_select_response, state, NULL);
    g_free(req_path);
    g_free(token);
}

static void
on_create_session_response(GDBusConnection *conn, const gchar *sender, const gchar *path, const gchar *iface,
                           const gchar *signal, GVariant *params, gpointer user_data) {
    ScreencastWebRTCState *state = user_data;
    guint32                response_code;
    GVariant              *results;
...
static void
create_session(ScreencastWebRTCState *state) {
    ...
    state->session_path =
        g_strdup_printf("%s/session/%s/%s", PORTAL_OBJECT_PATH, state->sanitized_name, state->session_token);
    ...
    GError   *error = NULL;
    GVariant *ret   = g_dbus_connection_call_sync(state->connection, PORTAL_BUS_NAME, PORTAL_OBJECT_PATH,
    ...
    g_dbus_connection_signal_subscribe(state->connection, PORTAL_BUS_NAME, REQUEST_INTERFACE, "Response", req_path,
                                       NULL, G_DBUS_SIGNAL_FLAGS_NONE, on_create_session_response, state, NULL);
    g_free(req_path);
    g_free(token);
}
...
static void
process_sdp_answer(ScreencastWebRTCState *state, const gchar *json_input) {
...
static gboolean
on_stdin_input(GIOChannel *channel, GIOCondition condition, gpointer user_data) {
    ScreencastWebRTCState *state = user_data;
    gchar                 *line  = NULL;
...
void
screencast_webrtc_tutorial(int argc, char *argv[]) {
