...
static void on_ice_gathering_state_change(GstElement* webrtc, guint mlineindex,
                                          gchar* candidate,
                                          gpointer user_data) {
...
static void start_stream(guint32 id, ScreencastWebRTCState* state) {
  ...
  char* pipeline_str = g_strdup_printf(
      ...
      "pulsesrc device=GStreamer_Yayin.monitor do-timestamp=true "
      "buffer-time=200000 ! "
      "audioconvert ! "
      "audioresample ! "
      "opusenc ! "
  ...
  g_object_set(state->webrtcbin, "stun-server", "stun:stun.relay.metered.ca:80",
               NULL);
  ...
  g_signal_connect(state->webrtcbin, "notify::ice-gathering-state",
                   G_CALLBACK(on_ice_gathering_state_change), state);
