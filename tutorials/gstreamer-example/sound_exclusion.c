...
void
run_command_output(const gchar *cmd, gchar **result) {
    GError *error         = NULL;
    gchar  *stdout_output = NULL;
    gint    exit_status   = 0;

    gboolean success = g_spawn_command_line_sync(cmd, &stdout_output, NULL, &exit_status, &error);
...
get_excluded_sound() {
    ...
    gchar *cmd_null = g_strdup_printf("pactl load-module module-null-sink sink_name=%s "
    ...
    gchar *cmd_loop = g_strdup_printf("pactl load-module module-loopback source=%s.monitor sink=%s", VIRTUAL_SINK_NAME,
                                      original_sink);
    loop_module_id  = load_module(cmd_loop);
    g_free(cmd_loop);

    g_print("[*] Setting default sink to VIRTUAL SINK...\n");
    gchar *cmd_def = g_strdup_printf("pactl set-default-sink %s", VIRTUAL_SINK_NAME);
    run_command(cmd_def);
    g_free(cmd_def);
    ...
    g_print("\nAudio Sources:\n");
    run_command("pactl list sink-inputs | grep -E 'Sink Input #|application.name'");
    ...
    if (g_io_channel_read_line(channel, &input_line, &len, NULL, &err) == G_IO_STATUS_NORMAL) {
        if (input_line) {
            ...
            if (g_strcmp0(input_line, "exit") == 0) {
                ...
                while (*current != NULL) {
                    ...
                    if (app_id > 0) {
                        ...
                        gchar *move_cmd = g_strdup_printf("pactl move-sink-input %u %s", app_id, original_sink);
