...
void run_command_output(const gchar* cmd, gchar** result) {
  ...
  gboolean success = g_spawn_command_line_sync(cmd, &stdout_output, NULL,
                                               &exit_status, &error);
...
void get_excluded_sound() {
  ...
  if (g_strcmp0(original_sink, VIRTUAL_SINK_NAME) == 0) {
    g_printerr(
        "ERROR: Virtual sink is already the default. Please reset the "
        "system first.\n");
    return;
  }

  g_print("[*] Creating virtual sink...\n");
  gchar* cmd_null = g_strdup_printf(
  ...
  g_print("\n[Waiting for Command]\n");
  g_print(
      "1. 'exclude {id} {id}...' -> Move given IDs to physical card (Hide "
      "from broadcast)\n");
