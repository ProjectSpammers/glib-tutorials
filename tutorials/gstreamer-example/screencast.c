...
static void
start_stream(guint32 id, ScreencastState *state) {
    g_print("\n>>> Starting Recording Pipeline... Node ID: %d\n", id);
    GstElement *pipeline;
    gst_init(NULL, NULL);
    gchar *audio_device = get_default_monitor_source();

    char *pipeline_str = g_strdup_printf("matroskamux name=mux ! filesink location=%s "
...
static void
start_screencast(ScreencastState *state) {
...
static void
select_sources(ScreencastState *state) {
...
static void
create_session(ScreencastState *state) {
...
void
screencast_tutorial(int argc, char *argv[]) {
    ...
    state->output_path =
        output_file ? g_strdup(output_file) : g_build_filename(g_get_current_dir(), "capture.mkv", NULL);
    if (output_file)
        g_free(output_file);
